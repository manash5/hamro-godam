...
const SettingsPage = () => {
  const { register, handleSubmit, setValue, formState: { errors: formErrors }, reset, watch } = useForm({
    defaultValues: {
      firstName: '',
      lastName: '',
      email: '',
      ...
      try {
        ...
        if (token) {
          try {
            ...
            userId = payload.user?._id || payload.user?.id || payload._id || payload.id || null;
          } catch (err) {
  ...
  const validateForm = (data) => {
    ...
    if (isEditing) {
      ...
      if (data.newPassword && !isPasswordStrong) {
  ...
  const onSubmit = async (formData) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      Object.entries(validationErrors).forEach(([key, value]) => setValue(key, formData[key]));
      return;
    }
    setLoading(true);
    setApiError('');
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem('admin', payload.FirstName || 'User');
        localStorage.setItem('adminEmail', payload.email || 'user@example.com');
      } else {
  ...
  return (
    <div className="h-screen bg-slate-100 flex">
        <Sidebar/>
      <div className="min-w-6xl mx-auto mt-10 overflow-auto hide-scrollbar">
